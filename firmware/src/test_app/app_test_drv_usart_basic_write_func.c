/*******************************************************************************
  ...
  File Name:
   app_test_drv_usart_basic_write_func.c
...
#include "app_test_drv_usart_basic_write_func.h"
#include "tm_fut_if.h"
...
// *****************************************************************************
/* USART Driver Test Application Data
...
typedef struct
{
    ...
    TS_DRV_USART_BASIC_WRITE         *usartBasicWriteTCs;    
 ...
 * Test Scenario Name: TS_DRV_USART_BASIC_WRITE

 *  Summary: To test MPLAB Harmony USART dynamic driver basic write functionality
...
*/
TS_DRV_USART_BASIC_WRITE usartBasicWriteTestcase[] =  
{
/*

 * Description: DRV_USART_Write function writes given number of bytes to the 
    USART PC Console/Tester.
...
*/     
    {
        .testCaseId = "USART driver basic write test 1",
        .data[0]    = { 
...
*/     
    {
        .testCaseId = "USART driver basic write test 2",
        .data[0]    = { 
                        .className =  DATACLASS_D, 
                        .startIndex = 2, 
                        .endIndex = 2, 
...
*/     
    {
        .testCaseId = "USART driver basic write test 3",
        .data[0]    = { 
                        .className =  DATACLASS_D, 
                        .startIndex = 3, 
                        .endIndex = 3, 
...
static uint8_t numberOfTests = sizeof(usartBasicWriteTestcase)/sizeof(TS_DRV_USART_BASIC_WRITE);
...
/*******************************************************************************
  Function:
    bool APP_TestDrvUsartFuncStart ( SYS_MODULE_OBJ object )
    
  Remarks:
    See prototype in test_drv_tmr_functional.h.
*/

bool APP_TestDrvUsartBasicWriteFuncStart ( SYS_MODULE_OBJ object )
{
...
SYS_MODULE_OBJ APP_TestDrvUsartBasicWriteFuncInitialize ( const SYS_MODULE_INDEX index,
                                          const SYS_MODULE_INIT * const init )
{
    int i;
    DRV_USART_TEST_DATA                   *pObj        = &testDrvUsartFunc;
    SYS_MODULE_OBJ                        retvalue    = (SYS_MODULE_OBJ)&testDrvUsartFunc;
    DRV_USART_FUNCTIONAL_TEST_BASIC_WRITE_INIT_DATA   *initTest    = (DRV_USART_FUNCTIONAL_TEST_BASIC_WRITE_INIT_DATA *)init;
...
SYS_STATUS APP_TestDrvUsartBasicWriteFuncStatus (  SYS_MODULE_OBJ object )
{
...
void APP_TestDrvUsartBasicWriteFuncTasks ( SYS_MODULE_OBJ object, unsigned int index )
{
    uint32_t len =0;
    uint32_t total =0;
    ...
    switch ( pObj->state )
    {
        ...
        /* Send Test Case Config */
        case APP_TEST_DRV_USART_FUNC_STATE_SEND_CONFIG:
        {
            //if(TM_SUCCESS == TM_SendFTMPDataClass(CLASS_J, pObj->test.config_index, pObj->test.config_index))
            {
               pObj->state = APP_TEST_DRV_USART_FUNC_STATE_CONFIG_WAIT; 
            }
            break;
        }
        
        /* Wait for Test configuration response */
        case APP_TEST_DRV_USART_FUNC_STATE_CONFIG_WAIT:
        {
            if(TM_GetResponseStatus() == TM_ACK)
            {
                pObj->state = APP_TEST_DRV_USART_FUNC_STATE_SEND_SOT;
            }
            break;
        }        
        
        /* Send Start of test command */
        case APP_TEST_DRV_USART_FUNC_STATE_SEND_SOT:
        {
            if(TM_SUCCESS == TM_SendFTMPCommand(FTMP_HNDLR_SOT))
            {
               pObj->state = APP_TEST_DRV_USART_FUNC_STATE_SOT_WAIT; 
            }
            break;
        }
        
        /* Wait for start of test command response */
        case APP_TEST_DRV_USART_FUNC_STATE_SOT_WAIT:
        {
            if(TM_GetResponseStatus() == TM_ACK)
            {
                pObj->state = APP_TEST_DRV_USART_FUNC_STATE_SEND_TEST_INFO;
            }
            break;
        }        
        ...
        {
            /* Add test case scenario and id */
            //info.test_scenario = pObj->test.test_index;
            info.test_case_id  = subtestId;
            
            /* Add other stimulus information */
            info.test_data = &usartBasicWriteTestcase[subtestId].data[0];
            info.test_data_len = sizeof(usartBasicWriteTestcase[subtestId].data)/sizeof(CLASS_DATA_ITEMS);
            
            if(TM_SendTestCaseInfo(info))
            {
                pObj->state = APP_TEST_DRV_USART_FUNC_STATE_TEST_INFO_WAIT;
            }
            break;
        }
        
        /* Wait for test info response */
        case APP_TEST_DRV_USART_FUNC_STATE_TEST_INFO_WAIT:
        {
            if(TM_GetResponseStatus() == TM_ACK)
            {
                pObj->state = APP_TEST_DRV_USART_FUNC_STATE_STIMULUS_REQUEST;
            }
            break;
        }
        ...
        {
            if(TM_SUCCESS == TM_SendFTMPCommand(FTMP_HNDLR_STIM_REQ))
            {
               pObj->state = APP_TEST_DRV_USART_FUNC_STATE_STIMULUS_WAIT; 
            }
            break;            
        }
        
        /* Wait for stimulus request response */
        ...
        {     
            /* Get the Class data */
            DC_GetClassData(usartBasicWriteTestcase[subtestId].data[0], &buffer, (uint32_t *) &len);
            
            if(len != 0)
            {
                do
                {
                    if( DRV_USART_TRANSFER_STATUS_TRANSMIT_EMPTY & DRV_USART_TransferStatus(pObj->handle))
                    {
                        numBytes = DRV_USART_Write(pObj->handle, buffer + total, (len - total));
                        total += numBytes;
                    }
            ...
            pObj->state = APP_TEST_DRV_USART_FUNC_STATE_CHECK_STATUS;
            
            break;
        }

        case APP_TEST_DRV_USART_FUNC_STATE_CHECK_STATUS:
        {
            pObj->state     = APP_TEST_DRV_USART_FUNC_STATE_COMPLETED_LOG_RESULT;
            break;
        }
        
        case APP_TEST_DRV_USART_FUNC_STATE_COMPLETED_LOG_RESULT:
        {           
            if(TM_LogResulttoTester(TM_TESTER))
            {
                TEST_HasPassedSubtest(true);
                pObj->state = APP_TEST_DRV_USART_STATE_WAIT_FOR_LOG_RESULT;
            }
        }
        ...
        case APP_TEST_DRV_USART_STATE_WAIT_FOR_LOG_RESULT:
        {
            if(TM_GetResponseStatus() == TM_ACK)
            {
                pObj->state = APP_TEST_DRV_USART_FUNC_STATE_SEND_EOT;
            }
            break;             
        }
        ...
        {
            if(TM_SUCCESS == TM_SendFTMPCommand(FTMP_HNDLR_EOT))
            {
               pObj->state = APP_TEST_DRV_USART_FUNC_STATE_EOT_WAIT; 
            }
            break;            
        } 
        ...
        {           
            if(++subtestId < numberOfTests)
            {
                pObj->state = APP_TEST_DRV_USART_FUNC_STATE_SEND_SOT;
            }
            else
            {
                DRV_USART_Close(pObj->handle);
                
                pObj->state = APP_TEST_DRV_USART_FUNC_STATE_IDLE;
                TEST_HasCompleted();
            }            
            break;
        }
