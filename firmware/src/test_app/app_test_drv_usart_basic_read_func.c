/*******************************************************************************
  ...
  File Name:
   app_test_drv_usart_basic_read_func.c
...
#include "test_app/app_test_drv_usart_basic_read_func.h"
...
typedef struct
{
    ...
    TS_DRV_USART_BASIC_READ         *usartBasicReadTCs;    
 ...
 * Test Scenario Name: TS_DRV_USART_BASIC_READ

 *  Summary: To test MPLAB Harmony USART dynamic driver basic read functionality
...
*/
TS_DRV_USART_BASIC_READ usartBasicReadTestcase[] =  
{
/*

 * Description: DRV_USART_Read function reads given number of bytes from the 
...
*/  
    {
        .testCaseId = "USART Driver Basic Read Test case 1",
        .data[0]    = { 
...
*/                
    {
        .testCaseId = "USART Driver Basic Read Test case 2",
        .data[0]    = { 
                        .className =  DATACLASS_D, 
                        .startIndex = 2, 
                        .endIndex = 2, 
...
*/                
    {
        .testCaseId = "USART Driver Basic Read Test case 3",
        .data[0]    = { 
                        .className =  DATACLASS_D, 
                        .startIndex = 1, 
                        .endIndex = 1, 
...
static uint8_t numberOfTests = sizeof(usartBasicReadTestcase)/sizeof(TS_DRV_USART_BASIC_READ);
                                          ...
                                          const SYS_MODULE_INIT * const init )
{
    ...
    DRV_USART_FUNCTIONAL_TEST_BASIC_READ_INIT_DATA   *initTest    = (DRV_USART_FUNCTIONAL_TEST_BASIC_READ_INIT_DATA *)init;
...
void APP_TestDrvUsartBasicReadFuncTasks ( SYS_MODULE_OBJ object, unsigned int index )
{
    ...
    switch ( pObj->state )
    {
        ...
        case APP_TEST_DRV_USART_FUNC_STATE_SEND_TEST_INFO:
        {
            ...
            info.test_data = &usartBasicReadTestcase[subtestId].data[0];
            info.test_data_len = sizeof(usartBasicReadTestcase[subtestId].data)/sizeof(CLASS_DATA_ITEMS);
        ...
        case APP_TEST_DRV_USART_FUNC_STATE_STIMULUS_WAIT:
        {
            ...
            {                
                len = 0;
                memset((uint8_t *)&rbuffer[0], 0, sizeof(rbuffer));
        ...
        {
            /* Get the Class data */
            DC_GetClassData(usartBasicReadTestcase[subtestId].data[0], &buffer, (uint32_t *) &len);
            
            if(len != 0)
            {
                do{

                    if( DRV_USART_TRANSFER_STATUS_RECEIVER_DATA_PRESENT & DRV_USART_TransferStatus(pObj->handle))
                    {
                        numBytes = DRV_USART_Read(pObj->handle, &rbuffer[total], (len - total));
        ...
        { 
            ...
            /* Check the data received is valid or not */
            if( memcmp(rbuffer, buffer, len) == 0 )
            {                 
                if(TM_LogResulttoTester(TM_PASS))
                {
                ...
                {
                    pObj->state = APP_TEST_DRV_USART_FUNC_STATE_ERROR;
                }
            }
            else
            {               
                if(TM_LogResulttoTester(TM_FAIL))
                {
                    pObj->state = APP_TEST_DRV_USART_STATE_WAIT_FOR_LOG_RESULT;
                }
                else
